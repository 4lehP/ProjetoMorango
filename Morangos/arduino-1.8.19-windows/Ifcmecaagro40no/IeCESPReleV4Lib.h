...
String deviceID() {
  ...
  return "Esp32_" + hexStr(ESP.getChipId());
#else
  // ESP32 utiliza função getEfuseMac()
  return "Esp32_-" + hexStr(ESP.getEfuseMac());
#endif
}
...
String networkID() {
  ...
  if (deserializeJson(jsonConfig, file)) {
    ...
    // Sucesso na leitura
    char ssidchar[30];
    strlcpy(ssidchar, jsonConfig["ssid"]  | "", sizeof(ssid));
...
String fsSpaceStr() {
  ...
  int x=0;
  return "Total: " +
          String((SPIFFS.totalBytes()) / 1024.0, 1) + F("kb<br>") +
          "Usado: " +
          String(SPIFFS.usedBytes() / 1024.0, 1) + F("kb<br>") +
          "Disponível: " +
